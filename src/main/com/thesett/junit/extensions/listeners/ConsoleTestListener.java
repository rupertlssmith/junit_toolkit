 ...
 * 'F', for each test that passes, is in error or fails. It does this for every test result registered with the
 * framework, not just on the completion of each test method as the JUnit one does. It also uses a throttle to cap the
 * rate of dot drawing, as exessively high rates can degrade test performance without providing much usefull feedback to
 * the user. Unlike the JUnit dot drawing feedback, this one will correctly wrap lines when tests are run concurrently
 * (the rate capping ensures that this does not become a hot-spot for thread contention).
 *
 * <p/>Where rate capping causes the conflation of multiple requested dots into a single dot, the dot that is actually
 * drawn will be the worst result within the conflation period, that is, error is worse than fail which is worse than
 * pass.
 *
