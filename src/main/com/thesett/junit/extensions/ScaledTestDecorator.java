...
public class ScaledTestDecorator extends WrappedSuiteTestDecorator implements ShutdownHookable // TestDecorator
{
    ...
    /**
     * The stress levels or numbers of simultaneous threads to run the test in. The test is repeated at each of the
     * concurrency levels specified here. Defaults to 1 thread.
     */
    ...
    /** The test to run. */
    private final WrappedSuiteTestDecorator test;
        ...
        return new Thread(new Runnable()
            {
                public void run()
                {
                    ...
                    if (testResult != null)
                    {
                        ...
                        {
                            // Restore the interrupted state of the thread. Exception noted so can be ignored.
                            e = null;
    ...
    private void executeAndWaitForRunnables(Runnable[] runnables)
    {
        ...
        for (int j = 0; j < numThreads; j++)
        {
            ...
            {
                // Restore the interrupted state of the thread. Exception noted and ignored.
                e = null;
                Thread.currentThread().interrupt();
            }
        }
    ...
    private static class TestThreadHandler implements Runnable
    {
         ...
         *                   completion before moving on through the setup, test, teardown phases. The barrier should be
         *                   configured for the number of test threads.
         */
        ...
        public void run()
        {
            ...
            catch (BrokenBarrierException e)
            {
                ...
                // interrupt all the other threads running in the same scaled test. Exception noted and ignored.
                e = null;
                Thread.currentThread().interrupt();
            }
        }
