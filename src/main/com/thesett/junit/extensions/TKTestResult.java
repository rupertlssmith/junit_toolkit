...
public class TKTestResult extends TestResult
{
    ...
    private final ThreadLocal<ThreadLocalSettings> threadLocals = new ThreadLocal<ThreadLocalSettings>();
    ...
    /**
     * Callback method use to inform this result that a test was completed. This calculates how long the test took to
     * run, then delegates to the super class implementation.
     *
    ...
    /**
     * Tells this test result that it should stop running tests. Once this method has been called this test result will
     * not start any new tests, and any tests that use the timing controller will be passed interrupted exceptions, to
     * indicate that they should end immediately. Usually the caller of this method will introduce a short wait to allow
     * an opporunity for running tests to complete, before forcing the shutdown of the JVM.
     */
    ...
    {
        return "TKTestResult@" + Integer.toString(hashCode(), 16) + ": [ testCaseName = " + testCaseName + ", n = " +
            n + ", tkListeners = " + tkListeners + " ]";
    }
    ...
    /**
     * Holds things that need to be kept on a per thread basis for each test invocation, such as the test start time and
    ...
    /**
     * Provides an implementation of the {@link TimingController} interface that timing aware tests can use to call back
     * to reset timers, and register additional test timings.
     */
    private static class TimingControllerImpl implements TimingController
    {
         ...
         * threads but still identify itself correctly to {@link TKTestListener}s as being associated with the thread
         * that called the test method.
         */
        final long threadId;
