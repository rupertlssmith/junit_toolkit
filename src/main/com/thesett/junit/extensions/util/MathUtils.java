...
public class MathUtils
{
    ...
    public static final Pattern NAME_VALUE_PATTERN =
        Pattern.compile("(" + NAME_VALUE_REGEXP + ")|(" + NAME_VALUE_VARIATION_REGEXP + ")|(" +
    ...
    /**
     * Given a start and end and a number of steps this method generates a sequence of evenly spaced integer values,
     * starting at the start (inclusive) and finishing at the end (inclusive) with the specified number of values in the
     * sequence. The sequence returned may contain less than the specified number where the integer range between start
     * and end is too small to contain that many.
     *
     ...
     * values, starting at the start (inclusive) and finishing at the end (inclusive) with the specified number of
     * values in the sequence. An exponentially spaced sequence is one where the ratio between any two consecutive
    ...
    /**
     * Parses a string defintion of a sequence into an int array containing the sequence. The definition will conform to
     * the regular expression: "^(\[[0-9,]+\])(,samples=[0-9]+)?(,exp)?$". This splits it into three parts, an array of
     * integers, the optional sample count and the optional exponential flag.
     *
    ...
    /**
     * Parses a duration defined as a string, giving a duration in days, hours, minutes and seconds into a number of
     * milliseconds equal to that duration.
     *
    ...
    public static long parseDuration(String duration)
    {
        ...
        if (numGroups >= 2)
        {
            ...
            result +=
                (hoursString == null)
                ? 0 : (Long.parseLong(hoursString.substring(0, hoursString.length() - 1)) * 60 * 60 * 1000);
        }
        ...
        if (numGroups >= 4)
        {
            ...
            result +=
                (secondsString == null)
                ? 0 : (Long.parseLong(secondsString.substring(0, secondsString.length() - 1)) * 1000);
        }
    ...
    /**
     * The #toArray methods of collections cannot be used with primitive arrays. This loops over and array list of
     ...
     */
    private static int[] intListToPrimitiveArray(List<Integer> result)
    {
    ...
    /**
     * Rounds the specified floating point value to the nearest integer and adds it to the specified list of integers,
