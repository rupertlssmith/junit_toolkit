...
public class ThreadTestCoordinator
{
    ...
    /**
     * Holds the deadlock timeout after which threads are given a runtime exception to signal that a potential deadlock
     * may be happening.
     */
    ...
    /**
     * Adds a thread to this coordinator and assigns an id to it. The ids must be numbered sequentially from 0 and it is
     * up to the caller to do this.
     *
     ...
     *
     * @throws InterruptedException If the calling thread is interrupted whilst waiting for threads to complete.
     */
    public String joinAndRetrieveMessages() throws InterruptedException
    {
    ...
    /**
     * Sets a timeout to break out of potential deadlocks. If all threads are waiting for other threads to send them
     * continue events for longer than this timeout then the threads are all terminated.
     *
     * @param millis The minimum time to allow to pass before breaking out of any potential deadlocks.
     *
     * @todo  This has not been implemented yet. If a potential deadlock happens then the joinAndRetrieveMessages method
     *        should throw a PotentialDeadlockException.
     */
    ...
    boolean consumeAllowEvent(int[] threads, boolean otherWaitIsAllow, int callerId, TestRunnable caller)
    {
        ...
        while (true)
        {
                            ...
                            (state == Thread.State.TIMED_WAITING))
                    {
                        ...
                        else
                        {
                            log.debug("The waiting allower thread, " + allowerId +
                                ", is waiting on the coordinator so does not allow thread " + callerId +
                                " to continue.");
                        }
                    }
                }
            ...
            {
                // Rethrow this as a runtime exception because generally speaking test code does not want to have to
