 ...
 * of the threads will expose a bug. Such bugs can be hard to replicate as the exact execution ordering of threads is
 * not usually controlled. This class adds some methods that allow threads to synchronize other threads, either allowing
 * them to run, or waiting for them to allow this thread to run. It also provides convenience methods to gather error
 * messages and exceptions from threads, which will often be reported in unit testing code.
 *
 * <p/>Coordination between threads is handled by the {@link ThreadTestCoordinator}. It is called through the
 * convenience methods {@link #allow} and {@link #waitFor}. Threads to be coordinated must be set up with the
 * coordinator and assigned integer ids. It is then possible to call the coordinator with an array of thread ids
 * requesting that those threads be allowed to continue, or to wait until one of them allows this thread to continue.
 * The otherwise non-deterministic execution order of threads can be controlled into a carefully determined sequence
 * using these methods in order to reproduce race conditions, dead locks, live locks, dirty reads, phantom reads, non
 ...
 * become blocked by the code under test. For example in testing for a dirty read (for example in database code), thread
 * 1 lets thread 2 perform a write but not commit it, then thread 2 lets thread 1 run and attempt to perform a dirty
 * read on its uncommitted write. Transaction synchronization code being tested against the possibility of a dirty write
 * may make use of snapshots in which case both threads should be able to read and write without blocking. It may make
 * use of explicit keys in which case thread 2 may become blocked on its write attempt because thread 1 holds a read
 * lock and it must wait until thread 1 completes its transaction before it can acquire this lock. The {@link #waitFor}
 * method accepts a boolean parameter to indicate that threads being blocked (other than on the coordinator) can be
 * interpreted the same as if the thread explicitly allows the thread calling waitFor to continue. Using this technique
 * a dirty read test could be written that works against either the snapshot or the locking implementation, allowing
 * both approaches to pass the test yet arranging for multiple threads to run against the implementation in such a way
 * that a potential dirty read bug is exposed.
 *
 ...
 *
 * @author Rupert Smith
 * @todo   The allow then waitFor operations are very often used as a pair. So create a method allowAndWait that
 *         combines them into a single method call.
 */
public abstract class TestRunnable implements Runnable
{
    ...
    /**
     * Provides a default implementation of the run method that allows exceptions to be thrown and keeps a record of
     * those exceptions. Defers to the {@link #runWithExceptions()} method to provide the thread body implementation and
     * catches any exceptions thrown by it.
     */
    public void run()
    {
        try
     ...
     * @param  otherWaitIsAllow If set to <tt>true</tt> if the threads being waited on are blocked other than on the
     *                          coordinator itself then this is to be interpreted as allowing this thread to continue.
     *
     * @return If the <tt>otherWaitIsAllow</tt> flag is set, then <tt>true</tt> is returned when the thread being waited
     *         on is found to be blocked outside of the thread test coordinator. <tt>false</tt> under all other
     *         conditions.
     */
